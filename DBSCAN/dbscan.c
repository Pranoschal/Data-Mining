...
void plot(int clid, int n) {
	for(int i=0; i<n; i++) {
		...
		switch(setofpoints[i].cluster_id) {
			...
			break;
			case 4: glColor3f(1,1,0);
			break;
			case 5: glColor3f(1,0,1);
			break;
			case 6: glColor3f(1,1,1);
			break;
			case 7: glColor3f(0,1,1);
			break;
			case 8: glColor3f(0.5,0,0);
			break;
			case 9: glColor3f(0,0.5,0);
			break;
			case 10: glColor3f(0,0,0.5);
			break;
			case 11: glColor3f(0.5,0.5,0);
			break;
		}
...
void draw() {
	...
	scanf("%d", &eps);
	printf("Enter value for MinPts: ");
	scanf("%d", &minpts);
	
	clid = 0;
	for(int i=0; i<n; i++) {
		if(setofpoints[i].cluster_id == -1) { //UNVISITED
